...
with tab1:
    ...
    <div class="pdf-box">
      ...
      <div style="color:rgba(255,255,255,0.65);font-size:0.82rem;">
        ...
        interprétation médicale · analyse du dataset · comparaison des 6 modèles · historique.
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if not PDF_AVAILABLE:
        st.warning("⚠️ Module `generate_report.py` introuvable. "
                   "Placez-le dans le même dossier que `app.py` et relancez.")

    elif st.session_state.last_prediction is None:
        st.info("ℹ️ Effectuez d'abord une prédiction ci-dessus pour activer l'export PDF.")
        ...
        with col_info:
            ...
            <div style="background:#f8f9fa;border:1px solid #e2e8f0;border-radius:12px;
              ...
              Modèle utilisé : <b>{lp['model_name']}</b>
            </div>
            """, unsafe_allow_html=True)
...
with tab2:
    ...
    with c5:
        ...
        fig = px.pie(target_counts, names='Diagnostic', values='Nombre',
                     ...
                     title=f"Répartition ({len(df_live)} patients au total)")
        st.plotly_chart(plotly_base(fig), use_container_width=True)
    with c6:
    ...
    if n_new > 0:
        st.markdown("---")
        st.subheader(f"🆕 Nouveaux patients ajoutés ({n_new})")
...
with tab3:
    ...
    with c2:
        ...
        colors_list  = ['#00c853','#e63946','#2196f3','#ff9800','#9c27b0']
        fig = go.Figure()
        for metric, color in zip(metrics_list, colors_list):
...
with tab6:
    ...
    st.download_button(
        "⬇️ Télécharger le dataset complet (CSV)",
